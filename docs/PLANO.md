...
- [x] **Fase 3 — Client-app: Cadastros.** Membros (nome/sobrenome, nascimento e início com precisão configurável ano/mês/dia — `PartialDateInput`), Cargos (filtro por tipo/busca, accordion de competências por bloco temático com seleção de afirmações/base de diálogo, atividades, prévia com contagem, bloqueio de exclusão com membros vinculados) e Competências (criar competência nova na biblioteca global — nome, bloco temático, base de diálogo com template de redação, N afirmações técnicas) implementados e testados ponta a ponta com Playwright. **Pendente:** editar/duplicar cargo e competência existentes; confirmação de exclusão hoje usa `window.confirm`/`alert`, não um diálogo do design system.
- [~] **Fase 4 — Client-app: Avaliações.** Os 3 fluxos (dialógica, tradicional, atividades) rodam sobre um runner único (`evaluation/EvaluationRunner.tsx` + `buildSections.ts`, que filtra as perguntas selecionadas do cargo por tipo — dialógica só pergunta dialógica, tradicional só afirmações, atividades usa a lista de atividades sem seções). Captura nota 1–5 (rótulos corretos por método — `packages/domain/src/scales.ts`), até 3 palavras-chave nas perguntas dialógicas, observação de seção + "o que posso fazer para melhorar/manter" (threshold configurável). Wizard de seleção (avaliador, avaliado por busca ou filtro de cargo, método) e tela de resultado (nota geral com legenda, desempenho por categoria expansível a nível de pergunta) implementados. Lista de avaliações com filtros por membro/método/período. Testado ponta a ponta: criar competência → criar cargo → criar 2 membros → rodar avaliação dialógica completa → resultado → aparece na lista. **Pendente:** tradicional e atividades não têm teste e2e próprio ainda (reaproveitam o mesmo runner testado via dialógica, mas não foram exercitados individualmente); tela de edição/reabertura de avaliação em andamento (hoje é tudo-ou-nada, sem salvar rascunho).
- [ ] **Fase 5 — Histórico/Comparação** (evolução por competência ao longo do tempo, acompanhamento dos combinados).
